...
npm run build             # Production build: runs migrations (if Postgres) + next build (Turbopack) + sitemap
...
**Build uses Turbopack (the default) — do NOT switch to `--webpack`.** The Vercel Blob client handler (`@payloadcms/storage-vercel-blob/client`) transitively imports Node-only modules (`pino` → `worker_threads`, `node:*`) that webpack cannot bundle for the client; Turbopack handles them. A `next build --webpack` will fail once the Blob plugin is registered.
...
- **Theming**: shadcn-style OKLCH CSS variables in `(frontend)/globals.css`. The brand identity is green/blue ("hub4youth.ai" — organic + AI). **Selectable brand themes**: 5 palettes (`teal`/`emerald`/`azure`/`indigo`/`amber`) defined as `[data-brand="…"]` blocks (light + dark) that re-tint `--primary`/`--ring`. The owner picks the active one via `NEXT_PUBLIC_BRAND_THEME`; `src/brand.ts` resolves it and the frontend layout sets `<html data-brand>`. The logo mark is `src/components/site/BrandMark.tsx` (fixed green/blue, theme-independent). Course-card icons map a stored string to a lucide icon in `src/components/site/CourseIcon.tsx`.
