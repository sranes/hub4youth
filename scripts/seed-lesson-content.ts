...
type LessonContent = { intro: string; points: string[]; takeaway: string }

function buildState(c: LessonContent) {
  return {
    root: {
      children: [
        para(c.intro),
...
async function main() {
  ...
  for (const [slug, lessonMap] of Object.entries(CONTENT)) {
    ...
    updatedLessons += matched
    console.log(`✓ ${slug}: wrote content for ${matched} lessons`)
  }

  console.log(`\nDone — ${updatedLessons} lessons updated.`)
  process.exit(0)
}
